...
func _process(delta: float) -> void:
	var temp = get_node("/root/root/PlayerBody") 
	# make sure every root node in every scene is called 
	#'root'. The stupid game engine can't handle finding 
	# the playercontroller itself so *I* have to do it 
	# for itasdlkf;jadsflkajsd	
